# StackBlend Studio Community

This repository is the StackBlend Studio Community, that will be your assets under the BSD 4-Clause license. ([read more on terms of service, section 2: Use License](https://www.softenstorm.com/stackblend-policy-and-terms)).

Please go to https://www.stackblend.org to get started. Please note that a dedicated GitHub account for StackBlend Studio Community is recommended while it is underdevelopment.
...
17. Connecting the repository with StackBlend Studio Community (see the instruction below).
18. Within StackBlend Studio Community, click save button to push changes to feature/YOUR_NEW_FEATURE_NAME" including new auto-generated files.
...
This boilerplate is designed for incremental update for any underlying supports of new features in StackBlend Studio Community. After StackBlend Studio Community has released a new version, you must merge the new changes into your project, to make it works on StackBlend.org. Please note that for all of the old versions, you must perform "git cherry-pick COMMIT_ID" for all of hotfixes, beginning with a prefix "Hotfix:" in the message, that may apply to your current using version.

1. Run "git stash".
2. Run "git tag -d 1.22.0".
3. Run "git fetch boilerplate --tags".
4. Run "git merge 1.22.0 --allow-unrelated-histories".
5. Run "git stash apply".

To list all of hotfixes:

1. Run "git tag -d 1.22.0".
2. Run "git fetch boilerplate --tags".
3. Run "git checkout 1.22.0".
